...
- **Data model** (`types/feed.ts`): `PodcastImage` interface (`href` required; optional `purpose`, `alt`, `aspectRatio`, `width`, `height`, `type`) + `podcastImages?: PodcastImage[]` on `Album`, `Track`, and `BaseChannelData`. `PODCAST_IMAGE_PURPOSES` is the canonical preset list (artwork/banner/canvas/social/publisher/circular/poster) feeding the UI dropdown. `PublisherFeed` is intentionally NOT wired up (out of scope) — the optional field on `BaseChannelData` keeps it type-compatible and it simply emits none.
...
- **Parse** (`xmlParser.ts`): `parsePodcastImages()` reads all `<podcast:image>` elements (normalizing fast-xml-parser's single-vs-array quirk) into the arrays. `'podcast:image'` is registered in BOTH `KNOWN_CHANNEL_KEYS` and `KNOWN_ITEM_KEYS` — load-bearing, or the tag would also be captured into `unknown*Elements` and double-emitted. Legacy `<podcast:images>` (plural) is still parsed into `trackArtUrl` for back-compat.
- **UI** (`PodcastImagesList.tsx`, mounted in `Editor.tsx` at feed + track level): artists paste a URL; `detectImageMetadata()` (`utils/imageMetadata.ts`) loads the image to auto-fill width/height/aspect-ratio/MIME and `suggestPurpose()` pre-selects a purpose. `detectImageMetadata` never rejects and has a 10s timeout (mirrors `audioUtils.getAudioDuration`). State flows through existing `UPDATE_ALBUM`/`UPDATE_TRACK` actions (no new reducer action). Known follow-up: the legacy `trackArtWidth`/`trackArtHeight` parser fields are now written-but-unread by the generator.
