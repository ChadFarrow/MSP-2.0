...
- `podping-verify.ts` - Check whether a podping actually landed on Hive; scans the last 100 `custom_json` ops on `PODPING_HIVE_ACCOUNT` for a `pp_*`/`podping` op whose payload mentions the feed URL. Rate-limited 60/hour per IP (the modal polls). Returns 501 when unconfigured and **502 when no Hive node is reachable — never `landed:false`**, so an RPC outage is never reported as a failed ping
