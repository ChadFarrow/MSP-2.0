...
- `check-feed-url.ts` - Reachability pre-flight: `GET /api/check-feed-url?url=` fetches a caller-supplied feed URL and returns a **verdict only, never the body** (`{ ok, status, contentType, looksLikeFeed, reason }`, `reason` ∈ `blocked | not-found | server-error | timeout | dns | not-xml | null`). Exists because a feed behind Cloudflare Bot Fight Mode / a WAF answers crawlers with 403, so it registers with PI but never indexes — a completely silent failure for the user (the `rollzmcguyver.com` case). Deliberately **not** `/api/proxy-feed`: that one enforces a podcast-host allowlist and would reject exactly the arbitrary self-hosted domains this check exists to test. Keeps proxy-feed's SSRF guard (`isPrivateHost`, now shared via `_utils/httpGuards.ts`) and rate-limits 30/hour per IP. Sends a **bot-shaped User-Agent on purpose** (`MSP-FeedCheck/1.0`) — a browser UA would sail past the very bot protection we're detecting. Uses a ranged GET rather than HEAD (many hosts 405 on HEAD but serve GET fine), and treats a 200 that returns HTML as `not-xml` (soft-404 / user pasted the site URL). **Two probes per check, and any block wins**: Cloudflare-style protection *scores* each request rather than applying a flat rule, so the same URL can serve a bare GET a challenge and a fuller one a 200 minutes apart (measured directly on `rollzmcguyver.com` — a single probe flipped between 403 and 206 within minutes, and `curl --http1.1` got 200 while `--http2` got 403, 8/8 each). One sample is a coin flip; two, biased toward warning, held at 6/6 `blocked` on that feed. Don't "simplify" this back to a single fetch. **This is advisory and must never gate a submit button** — it can be wrong (transient outage, or a host that blocks Vercel's egress but not PI's). Frontend: `src/utils/feedReachability.ts` (`checkFeedReachable`, `describeReachability` — the single source of the user-facing wording) + the `useFeedReachability` debounced hook (`src/hooks/`), wired into the three manual feed-URL inputs: `PodpingModal`, `SaveModal` (Submit to PodcastIndex mode), and `PublisherFeedReminderSection`. MSP-hosted URLs are skipped (`isMspUrl`) — we serve those ourselves. Any new manual feed-URL input that submits to PI/podping should add the hook next to its existing `getFeedUrlError()` check.
- **Reachability submit guard** — `guardFeedSubmission()` (`api/_utils/feedReachability.ts`) runs inline in `/api/pubnotify`, `/api/pisubmit` and `/api/podping`, the same three chokepoints where `getFeedUrlError()` is enforced and for the same reason: a per-input UI check can't cover the ~6 automatic submit paths (`publisherPublish.ts`, SaveModal's nsite follow-up, `PublisherFeedReminderSection`'s post-hosting ping, `CatalogFeedsSection`, `DownloadCatalogSection`, `Editor.tsx`) where nobody types a URL. Without it MSP reports "✅ Submitted!" for a feed Podcast Index can never fetch — the original `rollzmcguyver.com` bug. Rules, all load-bearing:
  - **Refuses only on `reason === 'blocked'`** (401/403/429). Every other verdict — timeout, DNS, 5xx, `not-xml` — **fails open**. Our probe being wrong, or Vercel's egress being unhappy, must never stop someone submitting a feed that is actually fine.
  - **`force` bypasses it entirely** (`?force=1` / `{ force: true }`, parsed by `wantsForce()`), surfaced as a **"Submit anyway" / "Send anyway"** button that appears only after a refusal. Needed because the verdict is probabilistic (see `check-feed-url.ts`) — a hard block would permanently trap a user whose host merely flapped.
  - **MSP-hosted URLs skip the probe** (`isMspHostedUrl()`) — we serve those.
  - **An overridden submit never reports clean success.** `SaveModal`, `PodpingModal` and `PublisherFeedReminderSection` all append "…but the feed was unreachable when we checked" instead of a bare ✅, so the override can't recreate the very false positive the guard exists to stop.
  - Refusal shape is `{ error, reachability }` at HTTP 400; the frontend detects it with `isGuardRefusal()` (`src/utils/feedReachability.ts`) rather than string-matching, and falls through to normal error handling for anything else. `GuardRefusal` is duplicated frontend-side (can't import from `api/`) — keep in sync.
  - Endpoint tests neutralize the guard via a hoisted `vi.mock` of `./_utils/feedReachability.js` so their mocked `fetch` queues still line up; the guard's real behaviour is covered in `api/_utils/feedReachability.test.ts`.
...
- **A successful podping does not mean the feed will index.** Podping only tells indexers to go look; if the feed's own host then answers their crawler with a 403 (Cloudflare Bot Fight Mode is the common culprit on WordPress sites), the feed registers in PI with `lastGoodHttpStatusTime: 0` and stays permanently blank. `PodpingModal` therefore shows the `/api/check-feed-url` reachability warning beside the URL field — but still lets the ping send, since the check can be wrong. When a user reports "I podpinged and nothing showed up", check `podcasts/byfeedurl` for `lastHttpStatus` before looking at anything in MSP.
