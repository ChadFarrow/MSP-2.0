...
### First-run onboarding wizard (`new-onboarding` branch)
A dead-simple guided flow that takes a brand-new artist from zero to a live hosted feed without touching the full editor. **Branch-only**: the `new-onboarding` branch is a long-lived, separately-deployed build (powers `new.musicsideproject.com`); it is intentionally NOT merged to `master`. Nostr-only auth throughout (no edit-token path).
- **Gate + entry points**: on first visit `App.tsx` renders `<OnboardingPage />` (`src/components/OnboardingPage.tsx`) when `!onboardingStorage.isComplete() && !wizardStorage.isComplete()`. Its "first-time" choice marks onboarding complete, calls `handleSwitchFeedType('artist')`, and opens the wizard (`setShowArtistWizard(true)`). The wizard is also reachable any time via **New → "New Artist (Guided)"** (`NewFeedChoiceModal`'s `onNewArtist`, shown in every feed mode — not gated to album). localStorage keys: `msp2-onboarding-complete` / `msp2-wizard-complete` (`onboardingStorage` / `wizardStorage` in `src/utils/storage.ts`); `handleDismiss` marks the wizard complete so it doesn't re-nag.
- **`<ArtistOnboardingWizard />`** (`src/components/ArtistOnboardingWizard.tsx`): a **full-page dialog** (`role="dialog" aria-modal`, `.onboarding-page*` classes in `App.css`, shared with `OnboardingPage`) — NOT a `ModalWrapper`. Provides its own Escape-to-close + mount focus. Modals opened over it (none currently) would need a z-index above `.onboarding-page` (1000). Four steps:
  1. **Login** — Nostr only: NIP-07 extension, bunker URI, and an **inline QR** (NIP-46 nostrconnect via `loginWithNip46(undefined, onUri)` rendered with `qrcode` onto a canvas). Auto-advances to step 2 on login. (There is no separate "More login options" modal — it was folded inline.)
  2. **Your Info** — required: artist name, album/single name, description, language (dropdown from `LANGUAGES` in `types/feed.ts`, ISO 639-1 so feeds never fail Apple validation). An "Optional — improves discovery and lets fans pay you" section adds: category (`ITUNES_CATEGORIES`), explicit (`Toggle`), **Lightning address** (V4V), owner name/email, keywords, and a support/funding link. The **artist npub is set automatically** from the logged-in Nostr user (no field) and emits as `<podcast:txt purpose="npub">`. InfoIcon tooltips here are short inline strings (not the more technical shared `FIELD_INFO`).
  3. **Upload Music** — **multi-track**: a styled label-button (`.wizard-upload-btn`) wraps `<input type="file" multiple>`; each selected file appends a `WizardTrack` (local `tracks` state array) whose title is pre-filled from the filename and duration auto-pulled from the file (`getAudioDuration(objectURL)` → `secondsToHHMMSS`, in parallel with the Blossom upload via `uploadMediaToBlossom`). Each track row has editable title/duration, an `<audio>` preview, remove (×), and retry-on-failure. **Album/Single art is required** (one cover for the whole release; per-track art is a later editor step). Validation: ≥1 track, each uploaded + titled + non-zero duration (`/[1-9]/`), plus artwork present.
  4. **Publish** — `handlePublish` builds an `Album` + `PublisherFeed` with cross-linked GUIDs generated **once** via `guidsRef` (so a failed→retried publish updates the same feeds instead of orphaning the album). Lightning address → value block (artist `split: 98` + `createSupportRecipients()` at 1/1; node-pubkey vs `lnaddress` auto-detected). Calls `checkSignerConnection()` pre-flight, then `hostBothOnMSP(album, publisher, nostrState.user.pubkey, onStep)` (hex `pubkey`, matching `ArtistPublishSection` — NOT `npub`). On a publisher-only failure it surfaces the already-live album URL + a non-blocking warning instead of a dead-end error; on success shows the feed URL + copy.
- **Upload-control styling** (`App.css`): `.wizard-upload-btn` — a themed label-button that wraps and hides the native file input, so the controls are large/readable on the dark theme and there's no misleading "No file selected" text (the wizard clears the input value after each upload, which the native control would otherwise render as "No file selected").
