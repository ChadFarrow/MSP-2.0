...
- `_utils/rateLimiter.ts` - In-memory fixed-window rate limiter over a **single shared `Map`**, so every caller must namespace its key or two endpoints silently share one bucket. Current consumers: `/api/verify-feed-url` (`verify-feed-url:`, 30/hr), `/api/podping-verify` (`podping-verify:`, 60/hr), `guardFeedSubmission()` (`feed-guard:`, 60/hr), `/api/auth/magic-link` (`magiclink:ip:` + `magiclink:email:`, 5 per 15 min) — and `/api/podping` (10/hr), which is the **one outlier: it keys on the bare IP with no prefix**. Harmless today because nothing else does, but any new endpoint calling `checkRateLimit(ip, …)` would silently share podping's bucket. Prefix new keys; ideally give podping one too.
