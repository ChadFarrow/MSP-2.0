...
export function downloadArtistFeedPackage(album: Album, publisherFeed: PublisherFeed): void {
  ...
  const instructions = [
    ...
    '  Upload both XML files to your own website, GitHub Pages, or S3.',
    '  Example:',
    ...
    '',
    '  Faster path: if you log in with Nostr, use the "Publish your feeds"',
    '  section on the New Artist page → "Host on MSP — album + publisher',
    '  (one click)" — MSP hosts both feeds at predictable URLs and',
    '  auto-submits them to Podcast Index in one click.',
    '',
