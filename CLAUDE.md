...
- The apostrophe rule matches the curly `’` (U+2019) and a pre-encoded `%27` as well as the ASCII `'`. A filename copied out of a word processor or a smart-quoting CMS carries the curly one, and without this it fell through to the generic non-ASCII rule and lost the duplicate-feed explanation — the *detail* shown is the first matching rule's, so ordering (whitespace → apostrophes → special chars → non-ASCII) is load-bearing. Hand-encoding `%27` doesn't stop Podcast Index forking the entry, so it gets the same message: rename the file at the host and 301 the old path.
