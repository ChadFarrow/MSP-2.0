...
- **`<OnboardingWizard />`** (`src/components/Onboarding/OnboardingWizard.tsx`, default export) — a **full-page dialog** (`.onboarding-page*` classes in `App.css`, shared with `OnboardingPage`; own Escape-to-close + mount focus). All wizard state/logic lives in the **`useOnboardingDraft`** hook (`src/components/Onboarding/useOnboardingDraft.ts`). Login UI is `NostrLoginPanel.tsx`.
