...
- `_utils/urlSafety.ts` - Shared SSRF guards: `isPrivateHost()` (literal-host string check, used by `proxy-feed`), `assertPublicHttpUrl()` (protocol + `isPrivateHost` + **DNS resolution** check — the resolution step is what stops an attacker pointing their own public hostname at cloud metadata, and it's load-bearing for `feedProbe` which has no allowlist behind it), and `getClientIp()`. PR #93 proposed a parallel `_utils/httpGuards.ts`; it was **not** merged — this file supersedes it, and the DNS step is why.
